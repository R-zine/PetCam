"""Command-line interface for PetCam ML."""
...
LOGGER = logging.getLogger("petcam_ml")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="petcam-ml",
        description="Capture, train, evaluate, and run PetCam posture models.",
    )
...
def _split_summary(manifest: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "samples": {
            split: {label: samples[split][label] for label in CLASS_NAMES} for split in split_names
        },
        "sessions": {
            split: {label: sessions[split][label] for label in CLASS_NAMES} for split in split_names
        },
    }
...
def _prepare_command(args: argparse.Namespace) -> int:
    ...
    data_root = args.data_root or (
        resolve_config_path(config["paths"]["data_dir"], config["_config_path"]) / "sessions"
    )
...
def _evaluate_command(args: argparse.Namespace) -> int:
    ...
    result = evaluate_checkpoint(
        ...
        num_workers=int(
            args.workers if args.workers is not None else config["training"]["workers"]
        ),
