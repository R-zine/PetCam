...
class Predictor:
        ...
        *,
        ...
        if set(class_to_idx) != REQUIRED_LABELS:
            raise CheckpointError(
                "PetCam checkpoints must contain exactly standing, lateral, and unknown; "
        ...
        model_kwargs = (
            ...
            and isinstance(metadata.get("model_kwargs", {}), Mapping)
            else {}
        )
    ...
    def predict_pil(self, image: Image.Image) -> Prediction:
        ...
        label = apply_confidence_threshold(raw_label, confidence, self.confidence_threshold)
        probabilities = {
...
def format_prediction(prediction: Prediction) -> str:
    ...
    lines.extend(
        f"{label}: {probability:.3f}" for label, probability in prediction.probabilities.items()
    )
    return "\n".join(lines)
