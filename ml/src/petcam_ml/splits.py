...
def _allocate_counts(count: int, ratios: Mapping[str, float], *, strict: bool) -> dict[str, int]:
    ...
    for name in sorted(
        SPLIT_NAMES, key=lambda item: (raw[item] - allocated[item], ratios[item]), reverse=True
    ):
    ...
    *,
    ...
    for session_id in sorted(records_by_session):
        ...
        for record in sorted(
            records_by_session[session_id], key=lambda item: str(_field(item, "path"))
        ):
            samples.append(
                SplitSample(
...
def load_split_manifest(path: str | Path) -> SplitManifest:
    ...
    try:
        for item in payload["samples"]:
            ...
            resolved_path = (
                raw_path if raw_path.is_absolute() else (source.parent / raw_path).resolve()
            )
            samples.append(
                SplitSample(
                    path=resolved_path,
