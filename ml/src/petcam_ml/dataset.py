    ...
    for metadata_path in metadata_paths:
        ...
        for frame in frames:
            ...
            if not image_path.is_file():
                report(
                    DatasetIssue(
                        "missing", image_path, f"Missing image referenced by metadata: {image_path}"
                    )
                )
                continue
...
def load_records_from_split_manifest(manifest_path: str | Path, split: str) -> list[ImageRecord]:
...
class PetCamDataset(Dataset[tuple[torch.Tensor, int]]):
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor, int]:
        ...
        if self.transform is None:
            raise RuntimeError("PetCamDataset requires a transform that returns a tensor")
