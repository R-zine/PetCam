"""Headless, resumable training for the RabbitCam posture classifier.
...
def _build_dataloaders(
    ...
    from .dataset import RabbitCamDataset
    from .transforms import build_transform

    paths = _mapping(config.get("paths"))
    training = _mapping(config.get("training"))
    manifest_value = paths.get("split_manifest") or _mapping(config.get("data")).get("split_manifest")
    ...
    train_dataset = RabbitCamDataset.from_manifest(
        manifest_path, split="train", transform=build_transform(train_transform_config, training=True)
    )
    validation_dataset = RabbitCamDataset.from_manifest(
        manifest_path,
        split="validation",
...
def _build_model(
    ...
    return factory(
        model_name=str(
            model_config.get("name", "mobilenetv4_conv_small.e3600_r256_in1k")
        ),
...
def _build_scheduler(
    optimizer: Optimizer, training: Mapping[str, Any], epochs: int
) -> Any | None:
    ...
    if scheduler_name in {"step", "steplr"}:
        return torch.optim.lr_scheduler.StepLR(
            optimizer,
            step_size=int(scheduler_options.get("step_size", training.get("scheduler_step_size", 10))),
...
def _compute_metrics(y_true: list[int], y_pred: list[int], class_names: list[str]) -> dict[str, Any]:
    from .metrics import compute_classification_metrics

    try:
        return dict(
            compute_classification_metrics(y_true, y_pred, class_names=class_names)
        )
    except TypeError:
...
def _log_epoch(
    ...
    writer.add_scalar(
        "validation/lateral_recall", float(metrics["lateral_recall"]), global_step
    )
...
def _prune_periodic(checkpoint_dir: Path, keep: int) -> None:
    periodic = sorted(
        checkpoint_dir.glob("periodic-step-*.pt"), key=lambda item: (item.stat().st_mtime_ns, item.name)
    )
...
def train(
    ...
    *,
    ...
    """Train or automatically resume a RabbitCam classifier.
    ...
    if fresh:
        ...
        checkpoint_dir.mkdir(parents=True, exist_ok=True)
    split_metadata = _snapshot_split_manifest(
        loaded_config, config_path, run_dir, fresh=fresh
    )
    _save_run_config(run_dir / "config.yaml", loaded_config)

    loaders = dict(dataloaders) if dataloaders is not None else _build_dataloaders(loaded_config, config_path)
    ...
    try:
        for epoch in range(resume.start_epoch, epochs):
            ...
            if skip_batches and loader_generator is not None and resume.loader_generator_state is not None:
            ...
            improved = monitored_value > best_metric if mode == "max" else monitored_value < best_metric
            if improved:
