...
def _validate(config: Mapping[str, Any]) -> None:
    ...
    fractions = [float(config["data"].get(name, -1)) for name in ("train_fraction", "validation_fraction", "test_fraction")]
...
def load_config(path: str | Path | None = None) -> dict[str, Any]:
    ...
    config = defaults if selected.resolve() == DEFAULT_CONFIG_PATH.resolve() else _deep_merge(defaults, _read_yaml(selected))
    _validate(config)
...
def save_config(config: Mapping[str, Any], path: str | Path) -> Path:
    ...
    serializable = {key: deepcopy(value) for key, value in config.items() if not str(key).startswith("_")}
...
except (FileNotFoundError, ValueError):  # pragma: no cover - only useful in malformed source installs
    DEFAULT_CONFIG = {}
