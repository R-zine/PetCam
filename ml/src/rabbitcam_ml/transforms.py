...
class ImageTransform:
    ...
    def __call__(self, image: Image.Image) -> torch.Tensor:
        ...
        if self.training and self.config.noise_std and random.random() < self.config.noise_probability:
...
def build_transform(
    ...
    resolved = config if isinstance(config, PreprocessConfig) else PreprocessConfig.from_mapping(config)
